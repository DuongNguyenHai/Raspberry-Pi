...
static uint8_t readi2c_register(uint8_t addr, uint8_t reg) {
...
static void writei2c_register(uint8_t addr, uint8_t reg, uint8_t val) {
   Wire.beginTransmission(addr);
   Wire.write(reg);
   Wire.write(val);
   Wire.endTransmission();
...
static uint16_t date2days(uint16_t y, uint8_t m, uint8_t d) {
   ...
   for (uint8_t i = 1; i < m; ++i)
      days += pgm_readbyte(daysInMonth + i - 1);
   if (m > 2 && y % 4 == 0)
      ++days;
...
DateTime::DateTime (uint32_t t) {
   ...
   for (m = 1; ; ++m) {
     uint8_t daysPerMonth = pgm_readbyte(daysInMonth + m - 1);
     if (leap && m == 2)
         ++daysPerMonth;
...
bool RTC_DS3231::lostPower(void) {
  return (readi2c_register(DS3231_ADDRESS, DS3231_STATUSREG) >> 7);
}
...
void RTC_DS3231::adjust(const DateTime& dt) {
   ...
   Wire.write(bin2bcd((uint8_t)0));
   Wire.write(bin2bcd(dt.day()));
   Wire.write(bin2bcd(dt.month()));
   Wire.write(bin2bcd(dt.year() - 2000));
   Wire.endTransmission();

   uint8_t statreg = readi2c_register(DS3231_ADDRESS, DS3231_STATUSREG);
   statreg &= ~0x80; // flip OSF bit
   writei2c_register(DS3231_ADDRESS, DS3231_STATUSREG, statreg);
}
...
void RTC_DS3231::writeSqwPinMode(Ds3231SqwPinMode mode) {
   uint8_t ctrl;
   ctrl = readi2c_register(DS3231_ADDRESS, DS3231_CONTROL);
   ...
   writei2c_register(DS3231_ADDRESS, DS3231_CONTROL, ctrl);
}

uint8_t RTC_DS3231::readRegControl() {
   return readi2c_register(DS3231_ADDRESS, DS3231_CONTROL);
}

bool RTC_DS3231::setAlarm(uint8_t alarm) {
   clearAlarm(alarm);
   uint8_t ctrl = readi2c_register(DS3231_ADDRESS, DS3231_CONTROL);
   ctrl |= 0x04;  // turn on INTCN
...
bool RTC_DS3231::isAlarmEnable() {
   return  (readi2c_register(DS3231_ADDRESS, DS3231_CONTROL) & 0x04) ? true : false;
}
...
uint8_t RTC_DS3231::readRegStatus() {
   return readi2c_register(DS3231_ADDRESS, DS3231_STATUSREG);
}

uint8_t RTC_DS3231::readRegister(uint8_t reg) {
   return readi2c_register(DS3231_ADDRESS, reg);
}

bool RTC_DS3231::isAlarmSet(uint8_t alarm) {
   return  (readi2c_register(DS3231_ADDRESS, DS3231_CONTROL) & alarm) ? true : false;
}

bool RTC_DS3231::isAlarmRing(uint8_t alarm) {
   return (readi2c_register(DS3231_ADDRESS, DS3231_STATUSREG) & alarm) ? true : false;
}

bool RTC_DS3231::clearAlarm(uint8_t alarm) {
   uint8_t regStatus = readi2c_register(DS3231_ADDRESS, DS3231_STATUSREG);
   if(alarm==0)
      regStatus &= ~0x03;
   else
      regStatus &= ~alarm;
   writei2c_register(DS3231_ADDRESS, DS3231_STATUSREG, regStatus);
   return true;
}
